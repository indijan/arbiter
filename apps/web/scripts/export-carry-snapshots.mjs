...
function parseArgs(argv) {
  const args = {
    hours: 24,
    output: "fixtures/carry-historical-export.json",
    exchanges: ["bybit", "okx"]
  };
...
async function fetchRows(hours, exchanges) {
  ...
  url.searchParams.set(
    "select",
    "ts,exchange,symbol,spot_bid,spot_ask,perp_bid,perp_ask,funding_rate"
  );
...
async function main() {
  ...
  const snapshots = rows
    ...
    .map((row) => ({
      ...
      symbol: row.symbol,
      spot_bid: Number(row.spot_bid),
      spot_ask: Number(row.spot_ask),
      perp_bid: Number(row.perp_bid),
      perp_ask: Number(row.perp_ask),
      funding_rate: Number(row.funding_rate)
    }));
