...
function meanReversionExitAt(post, entry, model) {
  if (!post.length) return { exit: entry, reason: 'single_snapshot' };
  const entryNet = taker(entry);
  const fixed = model.match(/^mr_exit_after_(\d+)m$/);
  if (fixed) {
    const horizon = t(entry) + Number(fixed[1]) * 60_000;
    return { exit: post.filter((r) => t(r) <= horizon).at(-1) ?? entry, reason: model };
  }
  const bracket = model.match(/^mr_tp(\d+)_sl(\d+)$/);
  if (bracket) {
    const tp = Number(bracket[1]); const sl = Number(bracket[2]);
    const tpRow = post.find((r) => entryNet - taker(r) >= tp);
    const slRow = post.find((r) => entryNet - taker(r) <= -sl);
    const tpTs = tpRow ? t(tpRow) : Infinity; const slTs = slRow ? t(slRow) : Infinity;
    if (tpTs <= slTs && tpRow) return { exit: tpRow, reason: 'spread_closed_take_profit' };
    if (slRow) return { exit: slRow, reason: 'spread_expanded_stop_loss' };
    return { exit: post.at(-1), reason: 'window_end' };
  }
  return { exit: post.at(-1), reason: 'window_end' };
}
...
function runSearch(rows, label) {
  ...
  for (const [pairKey, timelineRaw] of grouped.entries()) {
    ...
    const meanReversionPolicies = [
      { policy: 'mr_high_spread_5bps', ok: (r) => taker(r) >= 5 && persistence >= 3 },
      { policy: 'mr_high_spread_8bps', ok: (r) => taker(r) >= 8 && persistence >= 3 },
      { policy: 'mr_high_spread_stable_5bps', ok: (r) => taker(r) >= 5 && st >= 45 && persistence >= 3 },
      { policy: 'mr_extreme_spread_any_stability', ok: (r) => taker(r) >= 10 && persistence >= 2 },
      { policy: 'mr_exclude_bad_pairs_5bps', ok: (r) => taker(r) >= 5 && persistence >= 3 && !(r.symbol === 'BTCUSD' && r.exchange === 'bybit_coinbase') && !(r.symbol === 'BNBUSD' && r.exchange === 'bybit_kraken') }
    ];
    ...
    for (const entry of timeline) for (const p of meanReversionPolicies) { if (!p.ok(entry)) continue; const post = timeline.filter((r) => t(r) >= t(entry)); for (const model of meanReversionModels) { const { exit, reason } = meanReversionExitAt(post, entry, model); trials.push({ window: label, policy: p.policy, model, symbol: entry.symbol, exchange: entry.exchange, pair: pairKey, ts: entry.ts, maker_bps: round(maker(entry)), taker_bps: round(taker(entry)), stability: st, persistence, lifetime_minutes: round(lifetimeMin, 1), edge_bucket: bucket(taker(entry), [10, 5, 2, 0], ['edge_10_plus', 'edge_5_10', 'edge_2_5', 'edge_0_2', 'edge_non_positive']), stability_bucket: bucket(st, [75, 60, 45, 30], ['stability_75_plus', 'stability_60_75', 'stability_45_60', 'stability_30_45', 'stability_under_30']), exit_reason: reason, pnl_bps: round(taker(entry) - taker(exit)) }); } }
  }
